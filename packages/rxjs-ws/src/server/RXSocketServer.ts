import { Subject, Observable, merge, firstValueFrom } from 'rxjs';
...
export class RXSocketServer implements RXSocket {
  ...
  listen() {
    ...
    this.server.on('connection', (webSocket, request) => {
      ...
      socket.message$.subscribe((message: RXSocketMessage<any, any>) => {
        ...
        if (message.event.name === SUBSCRIBTION_EVENT_NAME) {
    ...
    this.server.on('listening', () =>
      (this.listening$ as Subject<void>).next()
    );
    this.server.on('close', () => (this.close$ as Subject<void>).next());
    this.server.on('error', (_: WebSocket, error: Error) =>
      (this.error$ as Subject<any>).next(error)
    );
  ...
  event<I = any, O = any>(event: EventName): RXServerSocketEvent<I, O> {
    return (
      this.events[event] ||
      (this.events[event] = new RXServerSocketEvent(this, event))
    );
  }
